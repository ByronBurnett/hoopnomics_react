...
const PlayerSelector = () => {
  ...
  const handlePageChange = async (page) => {
    ...
    await axios
      .get(
        `https://www.balldontlie.io/api/v1/players?search=${search}&page=${page}&per_page=10`
      )
      .then((res) => {
        setResults(res.data);
        console.log(results);
  ...
  const getPlayerSeasonalAverages = async (player, season) => {
    ...
    if (season) {
      await axios
        .get(
          `https://www.balldontlie.io/api/v1/season_averages?season=[]${season}&player_ids[]=${player.id}`
        )
        .then((res) => {
          return res.data;
        });
